...
def test_anonymous_user_redirect_to_login_when_posting_comment(client, news):
...
def test_authenticated_user_can_add_comment(user_client, news, user):
    url_news_detail = reverse('news:detail', args=[news.id])
    initial_count = Comment.objects.count()
...
def test_comment_with_profanity_is_rejected(user_client, news):
    url_news_detail = reverse('news:detail', args=[news.id])
    initial_count = Comment.objects.count()
...
def test_author_can_delete_own_comment(author_client, comment):
    ...
    url_news_detail = reverse('news:detail', args=[comment.news.id])
    initial_count = Comment.objects.count()
...
def test_non_author_cannot_delete_comment(user_client, comment):
    url_comment_delete = reverse('news:delete', args=[comment.id])
    initial_count = Comment.objects.count()
...
def test_non_author_cannot_edit_comment(user_client, comment):
    url_comment_edit = reverse('news:edit', args=[comment.id])
    initial_count = Comment.objects.count()
